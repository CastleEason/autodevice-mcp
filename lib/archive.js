...
const ZIP_LIST_SCRIPT = [
  ...
  "$archive = [IO.Compression.ZipFile]::OpenRead($env:AUTODEVICE_ARCHIVE_PATH)",
  "try {",
...
const ZIP_EXTRACT_SCRIPT = [
  "$ErrorActionPreference = 'Stop'",
  "Expand-Archive -LiteralPath $env:AUTODEVICE_ARCHIVE_PATH -DestinationPath $env:AUTODEVICE_DESTINATION_PATH -Force",
].join("; ");
...
async function defaultRunFile(command, args, options = {}) {
...
async function listZipEntries(archivePath, runFile) {
  const result = await runFile("powershell.exe", [
    ...
    env: { AUTODEVICE_ARCHIVE_PATH: archivePath },
  });
...
async function defaultExtract(asset, archivePath, destination, runFile) {
  ...
  if (asset.archiveType === "zip") {
    await runFile("powershell.exe", [
      ...
      env: {
        AUTODEVICE_ARCHIVE_PATH: archivePath,
        AUTODEVICE_DESTINATION_PATH: destination,
      },
    });
    return;
  }
...
async function isTrustedSystemSymlink(fileSystem, value, info) {
  const currentUid = process.getuid?.();
  if (currentUid === undefined || currentUid === 0 || info.uid !== 0) return false;
  const target = await fileSystem.stat(value);
...
async function ensureSafeDirectoryPath(fileSystem, directory) {
  ...
  for (const part of path.relative(root, resolved).split(path.sep).filter(Boolean)) {
    ...
    if (info.isSymbolicLink()) {
      if (await isTrustedSystemSymlink(fileSystem, current, info)) continue;
