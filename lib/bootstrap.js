...
async function recoverRuntime(root, expected, deps, logger) {
...
async function readLockOwner(lockPath, fileSystem) {
  ...
  try {
    ...
    return { owner, updatedAt: Math.max(owner.createdAt, ownerInfo.mtimeMs) };
  } catch (error) {
...
/** Keeps a live lock fresh so long downloads cannot be mistaken for stale work. */
function startLockHeartbeat(lockPath, deps, lockStaleMs, logger) {
  const interval = Math.max(1000, Math.min(30_000, Math.floor(lockStaleMs / 3)));
  const timer = setInterval(() => {
    const now = new Date(deps.now());
    deps.fileSystem.utimes(path.join(lockPath, LOCK_OWNER_FILE), now, now).catch((error) => {
...
async function releaseLock(lockPath, owner, deps, logger) {
  const current = await readLockOwner(lockPath, deps.fileSystem);
  if (!current || current.owner?.token !== owner.token) {
    throw runtimeError("runtime_lock_lost", `Runtime lock ownership changed before release: ${lockPath}`);
  }
...
async function publishRuntime(root, temporaryRoot, backupRoot, fileSystem) {
...
async function rollbackInvalidPublication(root, backupRoot, failedRoot, movedExisting, fileSystem) {
...
async function ensureRuntime(options = {}) {
  ...
  try {
    ...
    const stopHeartbeat = startLockHeartbeat(lockPath, deps, lockStaleMs, logger);
    try {
      const afterLock = await recoverRuntime(root, expected, deps, logger);
      ...
      try {
        ...
        let movedExisting;
        try {
          movedExisting = await publishRuntime(root, temporaryRoot, backupRoot, deps.fileSystem);
        } catch (error) {
        ...
        if (movedExisting) await cleanupGenerations([backupRoot], deps.fileSystem, logger);
        return published;
      } finally {
        if (temporaryCreated) {
      ...
      stopHeartbeat();
      await releaseLock(lockPath, owner, deps, logger);
    }
  } catch (error) {
